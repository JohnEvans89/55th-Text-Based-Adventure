...
try:
    ...
    if code.co_filename.startswith('<frozen'):
        ...
        report('Debugger warning: It seems the debugger cannot find os.path.realpath.__code__.co_filename (%s).',
               code.co_filename)
...
def create_source_reference_for_linecache(server_filename):
    ...
    pydev_log.debug('Created linecache id source reference: %s for server filename: %s', source_reference,
                    server_filename)
...
def create_source_reference_for_frame_id(frame_id, original_filename):
    source_reference = _next_source_reference()
    pydev_log.debug('Created frame id source reference: %s for frame id: %s (%s)', source_reference, frame_id,
                    original_filename)
        ...
        try:
            ...
            for eclipse_prefix, server_prefix in paths_from_eclipse_to_python:
                if translated_normalized.startswith(eclipse_prefix):
                    ...
                    if DEBUG_CLIENT_SERVER_TRANSLATION:
                        pydev_log.critical('pydev debugger: sent to server: %s - matched prefix: %s', translated,
                                           eclipse_prefix)
                    break
            else:
        ...
        try:
            ...
            for i, (eclipse_prefix, python_prefix) in enumerate(paths_from_eclipse_to_python):
                if translated_normalized.startswith(python_prefix):
                    ...
                    if DEBUG_CLIENT_SERVER_TRANSLATION:
                        pydev_log.critical('pydev debugger: sent to client: %s - matched prefix: %s', translated,
                                           python_prefix)
            ...
            if translated not in _client_filename_in_utf8_to_source_reference:
                if path_mapping_applied:
                    ...
                    source_reference = _next_source_reference()
                    pydev_log.debug('Created source reference: %s for untranslated path: %s', source_reference,
                                    filename)
