...
class ValidationInfo(object):
    ...
    def verify_breakpoints(self, py_db, canonical_normalized_filename, template_breakpoints_for_file, template):
        ...
        self._canonical_normalized_filename_to_last_template_lines[
            canonical_normalized_filename] = valid_lines_frozenset, sorted_lines
        self._verify_breakpoints_with_lines_collected(py_db, canonical_normalized_filename,
                                                      template_breakpoints_for_file, valid_lines_frozenset,
                                                      sorted_lines)
    ...
    def _verify_breakpoints_with_lines_collected(self, py_db, canonical_normalized_filename,
        ...
        for line, template_bp in list(
                template_breakpoints_for_file.items()):  # Note: iterate in a copy (we may mutate it).
                ...
                if not valid:
                    ...
                    if new_line >= 0 and new_line not in template_breakpoints_for_file:
                        ...
                        if template_bp.add_breakpoint_result.error_code != PyDevdAPI.ADD_BREAKPOINT_NO_ERROR and template_bp.add_breakpoint_result.translated_line != new_line:
                            pydev_log.debug('Template breakpoint in %s in line: %s moved to line: %s',
                            ...
                            template_bp.on_changed_breakpoint_state(template_bp.breakpoint_id,
                                                                    template_bp.add_breakpoint_result)
                    else:
                        if template_bp.add_breakpoint_result.error_code != PyDevdAPI.ADD_BREAKPOINT_INVALID_LINE:
                            pydev_log.debug('Template breakpoint in %s in line: %s invalid (valid lines: %s)',
                                            canonical_normalized_filename, line, valid_lines_frozenset)
