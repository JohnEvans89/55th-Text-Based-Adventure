...
def send_concurrency_message(event_class, time, name, thread_id, type, event, file, line, frame, lock_id=0,
                             parent=None):
                    ...
                    if hasattr(frame, "f_back") and frame.f_back is not None:
                        ...
                        if "self" in back.f_locals:
                            if isinstance(back.f_locals["self"], threading.Thread):
                                ...
                                if send_massage:
                                    send_concurrency_message("threading_event", event_time, "Thread", my_thread_id,
                                                             "thread",
                                                             "stop", my_back.f_code.co_filename, my_back.f_lineno,
                                                             my_back, parent=None)
                ...
                if self_obj.__class__ == ObjectWrapper:
                    ...
                    if method_name == "__init__":
                        send_concurrency_message("threading_event", event_time, t.name, get_thread_id(t), "lock",
                                                 method_name, back.f_code.co_filename, back.f_lineno, back,
                    ...
                    if "attr" in frame.f_locals and \
                        ...
                        if real_method in ("put_end", "get_end"):
                            ...
                            send_concurrency_message("threading_event", event_time, t.name, get_thread_id(t), "lock",
                                                     "release", back.f_code.co_filename, back.f_lineno, back,
                                                     lock_id=str(id(self_obj)))
        ...
        if "self" in frame.f_locals:
            ...
            if isinstance(self_obj, asyncio.Task):
                ...
                if method_name == "set_result":
                    ...
                    send_concurrency_message("asyncio_event", event_time, task_name, task_name, "thread", "stop",
                ...
                if method_name == "__init__":
                    task_id = id(self_obj)
                    task_name = self.task_mgr.get(str(task_id))
                    send_concurrency_message("asyncio_event", event_time, task_name, task_name, "thread", "start",
                                             frame.f_code.co_filename,
                                             frame.f_lineno, frame)
            ...
            if isinstance(self_obj, asyncio.Lock):
                if method_name in ("acquire", "release"):
                    ...
                    if method_name == "acquire":
                        if not self_obj._waiters and not self_obj.locked():
                            send_concurrency_message("asyncio_event", event_time, task_name, task_name, "lock",
                                                     method_name + "_begin", frame.f_code.co_filename, frame.f_lineno,
                                                     frame, lock_id=str(id(self_obj)))
                        if self_obj.locked():
                            method_name += "_begin"
                        else:
                            method_name += "_end"
                    ...
                    send_concurrency_message("asyncio_event", event_time, task_name, task_name, "lock",
                                             method_name, frame.f_code.co_filename, frame.f_lineno, frame,
                                             lock_id=str(id(self_obj)))
            ...
            if isinstance(self_obj, asyncio.Queue):
                if method_name in ("put", "get", "_put", "_get"):
                    ...
                    if method_name == "put":
                        send_concurrency_message("asyncio_event", event_time, task_name, task_name, "lock",
                                                 "acquire_begin", frame.f_code.co_filename, frame.f_lineno, frame,
                                                 lock_id=str(id(self_obj)))
                    elif method_name == "_put":
                        send_concurrency_message("asyncio_event", event_time, task_name, task_name, "lock",
                                                 "acquire_end", frame.f_code.co_filename, frame.f_lineno, frame,
                                                 lock_id=str(id(self_obj)))
                        send_concurrency_message("asyncio_event", event_time, task_name, task_name, "lock",
                                                 "release", frame.f_code.co_filename, frame.f_lineno, frame,
                                                 lock_id=str(id(self_obj)))
