    ...
    def can_be_executed_by(self, _thread_id):
        ...
        try:
            self._cmd = py_db.cmd_factory.make_get_thread_stack_message(
                ...
                start_frame=self._start_frame, levels=self._levels)
        finally:
            frame = None
            t = None
...
class InternalGetArray(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        try:
            ...
            xml = pydevd_vars.table_like_struct_to_xml(var, self.name, self.roffset, self.coffset, self.rows, self.cols,
                                                       self.format)
            cmd = dbg.cmd_factory.make_get_array_message(self.sequence, xml)
            dbg.writer.add_command(cmd)
        except:
            cmd = dbg.cmd_factory.make_error_message(self.sequence,
                                                     "Error resolving array: " + get_exception_traceback_str())
            dbg.writer.add_command(cmd)
...
def internal_change_variable(dbg, seq, thread_id, frame_id, scope, attr, value):
    ...
    try:
        ...
        cmd = dbg.cmd_factory.make_error_message(seq, "Error changing variable attr:%s expression:%s traceback:%s" % (
...
def internal_get_frame(dbg, seq, thread_id, frame_id):
    ...
    try:
        ...
        cmd = dbg.cmd_factory.make_error_message(seq,
                                                 "Error resolving frame: %s from thread: %s" % (frame_id, thread_id))
        dbg.writer.add_command(cmd)


def internal_get_smart_step_into_variants(dbg, seq, thread_id, frame_id, start_line, end_line,
                ...
                if variant.children_variants:
                    for child_variant in variant.children_variants:
                        ...
                        if child_variant.call_order > 1:
                            targets.append(StepInTarget(id=target_id, label='%s (call %s)' % (
                    ...
                    if variant.call_order > 1:
                        targets.append(
                            StepInTarget(id=target_id, label='%s (call %s)' % (variant.name, variant.call_order), ))
                    else:
                ...
                if context == 'hover':  # In a hover it doesn't make sense to do an exec.
                    ...
                    try:
                        ...
                        if context == 'repl':
                            ...
                            _evaluate_response_return_exception(py_db, request, eval_result.etype, eval_result.result,
                                                                eval_result.tb)
                            return
...
@silence_warnings_decorator
def internal_evaluate_expression(dbg, seq, thread_id, frame_id, expression, is_exec, trim_if_too_big,
                                 attr_to_set_result):
    ''' gets the value of a variable '''
    try:
        frame = dbg.find_frame(thread_id, frame_id)
...
def internal_set_expression_json(py_db, request, thread_id):
    ...
    if frame_tracker is None:
        # This is not really expected.
        _set_expression_response(py_db, request, result,
                                 error_message='Thread id: %s is not current thread id.' % (thread_id,))
        return
...
def build_exception_info_response(dbg, thread_id, thread, request_seq, set_additional_thread_info,
                                  iter_visible_frames_info, max_frames):
    '''
                ...
                while frames_list is not None and len(frames_list):
                    ...
                    if IS_PY311_OR_GREATER:
                        ...
                        for filename_in_utf8, lineno, method_name, line_text, line_col_info in frames[-max_frames:]:
                            frame_summary = traceback.FrameSummary(filename_in_utf8, lineno, method_name,
                                                                   line=line_text)
...
def internal_get_exception_details_json(dbg, request, thread_id, thread, max_frames, set_additional_thread_info=None,
                                        iter_visible_frames_info=None):
    ''' Fetch exception details
    '''
    try:
        response = build_exception_info_response(dbg, thread_id, thread, request.seq, set_additional_thread_info,
                                                 iter_visible_frames_info, max_frames)
    except:
        exc = get_exception_traceback_str()
...
class InternalGetBreakpointException(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        try:
            ...
            cmd = dbg.cmd_factory.make_send_breakpoint_exception_message(self.sequence,
                                                                         self.exc_type + "\t" + callstack)
            dbg.writer.add_command(cmd)
        except:
            exc = get_exception_traceback_str()
            sys.stderr.write('%s\n' % (exc,))
...
class InternalSendCurrExceptionTrace(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        try:
            cmd = dbg.cmd_factory.make_send_curr_exception_trace_message(dbg, self.sequence, self.thread_id,
                                                                         self.curr_frame_id, *self.arg)
            del self.arg
            dbg.writer.add_command(cmd)
        except:
            exc = get_exception_traceback_str()
...
class InternalSendCurrExceptionTraceProceeded(InternalThreadCommand):
    ...
    def do_it(self, dbg):
        try:
            ...
            sys.stderr.write('%s\n' % (exc,))
            cmd = dbg.cmd_factory.make_error_message(self.sequence,
                                                     "Error Sending Current Exception Trace Proceeded: " + exc)
            dbg.writer.add_command(cmd)
