...
from _pydevd_bundle.pydevd_comm import (InternalGetThreadStack, internal_get_completions,
                                        ...
                                        internal_get_description, internal_get_frame, internal_evaluate_expression,
                                        InternalConsoleExec,
                                        internal_get_variable_json, internal_change_variable,
...
from _pydevd_bundle.pydevd_comm_constants import (CMD_THREAD_SUSPEND, CMD_STEP_INTO_MY_CODE, CMD_STOP_ON_START,
                                                  CMD_SMART_STEP_INTO)
...
class PyDevdAPI(object):
    ...
    def request_stack(self, py_db, seq, thread_id, fmt=None, timeout=.5, start_frame=0, levels=0):
        ...
        internal_get_thread_stack = InternalGetThreadStack(
            seq, thread_id, py_db, set_additional_thread_info, fmt=fmt, timeout=timeout, start_frame=start_frame,
            levels=levels)
    ...
    def request_smart_step_into(self, py_db, seq, thread_id, offset, child_offset):
        ...
        if t:
            py_db.post_method_as_internal_command(
                thread_id, internal_smart_step_into, thread_id, offset, child_offset,
                set_additional_thread_info=set_additional_thread_info)
        elif thread_id.startswith('__frame__:'):
    ...
    def add_breakpoint(
        ...
        if not translated_absolute_filename.startswith('<'):
            ...
            if (
                    ...
                    py_db.apply_files_filter(self._DummyFrame(translated_absolute_filename),
        ...
        if breakpoint_type == 'python-line':
            added_breakpoint = LineBreakpoint(
                breakpoint_id, line, condition, func_name, expression, suspend_policy, hit_condition=hit_condition,
            ...
            if plugin is not None:
                add_plugin_breakpoint_result = plugin.add_breakpoint(
                    ...
                    breakpoint_id, line, condition, expression, func_name, hit_condition=hit_condition,
                    is_logpoint=is_logpoint,
        ...
        if py_db.plugin is not None:
            ...
            py_db.plugin.after_breakpoints_consolidated(py_db, canonical_normalized_filename, id_to_pybreakpoint,
    ...
    def remove_breakpoint(self, py_db, received_filename, breakpoint_type, breakpoint_id):
        ...
        if file_to_id_to_breakpoint is None:
            ...
            try:
                ...
                py_db.consolidate_breakpoints(canonical_normalized_filename, id_to_pybreakpoint,
                                              file_to_line_to_breakpoints)
                if py_db.plugin is not None:
                    py_db.has_plugin_line_breaks = py_db.plugin.has_line_breaks()
                    py_db.plugin.after_breakpoints_consolidated(py_db, canonical_normalized_filename,
    ...
    def setup_auto_reload_watcher(self, py_db, enable_auto_reload, watch_dirs, poll_target_time, exclude_patterns,
                                  include_patterns):
        py_db.setup_auto_reload_watcher(enable_auto_reload, watch_dirs, poll_target_time, exclude_patterns,
                                        include_patterns)
