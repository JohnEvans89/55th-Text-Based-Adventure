    ...
    if f_unhandled is not None:
        if f_unhandled.f_back is None and not force_only_unhandled_tracer:
            ...
            additional_info.top_level_thread_tracer_no_back_frames.append(
                top_level_thread_tracer)  # Hack for cython to keep it alive while the thread is alive (just the method in the SetTrace is not enough).
        else:
...
class ThreadTracer(object):
    ...
    def __call__(self, frame, event, arg):
        ...
        try:
            ...
            if frame_cache_key in cache_skips:
                if not is_stepping:
                    ...
                    if cache_skips.get(frame_cache_key) == 1:
                        ...
                        CMD_STEP_INTO, CMD_STEP_INTO_MY_CODE) and not _global_notify_skipped_step_in:
                            notify_skipped_step_in_because_of_filters(py_db, frame)

                        back_frame = frame.f_back
                        if back_frame is not None and pydev_step_cmd in (
                        CMD_STEP_INTO, CMD_STEP_INTO_MY_CODE, CMD_STEP_RETURN, CMD_STEP_RETURN_MY_CODE):
            ...
            file_type = py_db.get_file_type(frame,
                                            abs_path_canonical_path_and_base)  # we don't want to debug threading or anything related to pydevd
