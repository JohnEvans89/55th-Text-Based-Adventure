                    ...
                    if exception == SystemExit and main_debugger.ignore_system_exit_code(value):
                        ...
                        for exc_break, is_user_uncaught in check_excs:
                            ...
                            if main_debugger.exclude_exception_by_filter(exc_break, trace):
                                pydev_log.debug("Ignore exception %s in library %s -- (%s)" % (
                                ...
                                if not main_debugger.apply_files_filter(frame, frame.f_code.co_filename, True) \
                                        and (frame.f_back is None or main_debugger.apply_files_filter(frame.f_back,
                ...
                for check_trace_obj in (initial_trace_obj, trace_obj):
                    ...
                    if exc_lineno not in merged:  # Note: check on merged but update lines_ignored.
                        try:
                            ...
                            pydev_log.exception('Error in linecache.getline(%r, %s, f_globals)', absolute_filename,
                                                exc_lineno)
                            line = ''
        ...
        try:
            ...
            if frame.f_code.co_flags & 0xa0:  # 0xa0 ==  CO_GENERATOR = 0x20 | CO_COROUTINE = 0x80
                ...
                if event == 'line':
                    ...
                    if (
                            ...
                            step_cmd in (
                    CMD_STEP_OVER, CMD_STEP_RETURN, CMD_STEP_OVER_MY_CODE, CMD_STEP_RETURN_MY_CODE, CMD_SMART_STEP_INTO)
                    ):
                    ...
                    if frame.f_code.co_firstlineno == frame.f_lineno:  # Check line to deal with async/await.
                        function_breakpoint_on_call_event = main_debugger.function_breakpoint_name_to_breakpoint.get(
            ...
            if not is_exception_event:
                ...
                if info.pydev_state == 1:  # STATE_RUN = 1
                    ...
                    elif step_cmd in (CMD_STEP_OVER, CMD_STEP_RETURN, CMD_STEP_OVER_MY_CODE,
                                      CMD_STEP_RETURN_MY_CODE) and not self._is_same_frame(stop_frame, frame):
                        can_skip = True
                    ...
                    if can_skip:
                        ...
                        CMD_STEP_OVER, CMD_STEP_OVER_MY_CODE) and self._is_same_frame(stop_frame, frame.f_back):
            ...
            try:
                ...
                if breakpoint:
                    ...
                    if is_call and (frame.f_code.co_name in ('<lambda>', '<module>') or (
                            line == 1 and frame.f_code.co_name.startswith('<cell'))):
                ...
                if main_debugger.show_return_values:
                    if is_return and (
                            (info.pydev_step_cmd in (CMD_STEP_OVER, CMD_STEP_OVER_MY_CODE, CMD_SMART_STEP_INTO) and (
                            self._is_same_frame(stop_frame, frame.f_back))) or
                            (info.pydev_step_cmd in (CMD_STEP_RETURN, CMD_STEP_RETURN_MY_CODE) and (
                            self._is_same_frame(stop_frame, frame))) or
                            (info.pydev_step_cmd in (CMD_STEP_INTO, CMD_STEP_INTO_COROUTINE)) or
                            (
                                    info.pydev_step_cmd == CMD_STEP_INTO_MY_CODE
                                    and frame.f_back is not None
                                    and not main_debugger.apply_files_filter(frame.f_back,
                                                                             frame.f_back.f_code.co_filename, True)
                            )
                    ):
            ...
            try:
                    ...
                    if is_line:
                        if not info.pydev_use_scoped_step_frame:
                            if force_check_project_scope or main_debugger.is_files_filter_enabled:
                                stop = not main_debugger.apply_files_filter(frame, frame.f_code.co_filename,
                            ...
                            if force_check_project_scope or main_debugger.is_files_filter_enabled:
                                ...
                                if not not main_debugger.apply_files_filter(frame, frame.f_code.co_filename,
                                                                            force_check_project_scope):
                                    return None if is_call else NO_FTRACE
                        ...
                        if main_debugger.get_file_type(frame.f_back) == main_debugger.PYDEV_FILE:
                            ...
                            if force_check_project_scope or main_debugger.is_files_filter_enabled:
                                stop = not main_debugger.apply_files_filter(frame.f_back,
                                                                            frame.f_back.f_code.co_filename,
                                                                            force_check_project_scope)
                                if stop:
                    ...
                    if self._is_same_frame(stop_frame, frame) and is_return:
                        ...
                        if info.pydev_smart_child_offset != -1:
                            ...
                            if pydev_smart_parent_offset >= 0 and pydev_smart_step_into_variants:
                                ...
                                stop = get_smart_step_into_variant_from_frame_offset(back.f_lasti,
                                                                                     pydev_smart_step_into_variants) is \
                                       get_smart_step_into_variant_from_frame_offset(pydev_smart_parent_offset,
                        ...
                        if pydev_smart_child_offset >= 0 and pydev_smart_child_offset >= 0:
                            ...
                            if pydev_smart_parent_offset >= 0 and pydev_smart_step_into_variants:
                                ...
                                stop = children_variants and (
                                        get_smart_step_into_variant_from_frame_offset(back.f_lasti,
                                                                                      children_variants) is \
                                        get_smart_step_into_variant_from_frame_offset(pydev_smart_child_offset,
                                                                                      children_variants)
                                )
                ...
                if plugin_stop:
                    stopped_on_plugin = plugin_manager.stop(main_debugger, frame, event, self._args, stop_info, arg,
                                                            step_cmd)
                elif stop:
                    if is_line:
