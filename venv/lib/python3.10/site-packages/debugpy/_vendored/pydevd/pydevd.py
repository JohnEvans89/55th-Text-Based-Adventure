...
from _pydevd_bundle.pydevd_comm_constants import (CMD_THREAD_SUSPEND, CMD_STEP_INTO, CMD_SET_BREAK,
                                                  CMD_STEP_INTO_MY_CODE, CMD_STEP_OVER, CMD_SMART_STEP_INTO,
                                                  CMD_RUN_TO_LINE,
...
from _pydevd_bundle.pydevd_constants import (get_thread_id, get_current_thread_id,
                                             ...
                                             clear_cached_thread_id, INTERACTIVE_MODE_AVAILABLE, NULL,
                                             NO_FTRACE, IS_IRONPYTHON, JSON_PROTOCOL, IS_CPYTHON, HTTP_JSON_PROTOCOL,
                                             USE_CUSTOM_SYS_CURRENT_FRAMES_MAP, call_only_once,
...
from _pydevd_bundle.pydevd_trace_dispatch import (
    ...
    fix_top_level_trace_and_get_trace_func, USING_CYTHON)
...
from _pydevd_bundle.pydevd_concurrency_analyser.pydevd_concurrency_logger import ThreadingLogger, AsyncioLogger, \
    send_concurrency_message, cur_time
...
from _pydevd_bundle.pydevd_comm import (InternalConsoleExec,
                                        _queue, ReaderThread, get_global_debugger,
                                        set_global_debugger, WriterThread,
                                        start_client, start_server, InternalGetBreakpointException,
...
try:
    'dummy'.encode(
        'utf-8')  # Added because otherwise Jython 2.2.1 wasn't finding the encoding (if it wasn't loaded in the main thread).
except:
    pass
...
class PyDB(object):
    ...
    def setup_auto_reload_watcher(self, enable_auto_reload, watch_dirs, poll_target_time, exclude_patterns,
                                  include_patterns):
        try:
        ...
        try:
            ...
            if (self._is_libraries_filter_enabled or force_check_project_scope) and not self.in_project_scope(frame):
                ...
                if force_check_project_scope:
                    ...
                    pydev_log.debug_once('File not traced (not in project - force_check_project_scope): %s',
                                         original_filename)
                ...
                for thread_id in process_thread_ids:
                    ...
                    try:
                        while True:
                            ...
                            if not self.mpl_hooks_in_debug_console and isinstance(int_cmd,
                                                                                  InternalConsoleExec) and not self.gui_in_use:
                                # add import hooks for matplotlib patches if only debug console was started
                                try:
                                    self.init_matplotlib_in_debug_console()
                                    self.gui_in_use = True
                                except:
                                    pydev_log.debug("Matplotlib support in debug console failed",
    ...
    def add_break_on_exception(
        ...
        try:
            ...
            pydev_log.critical("Error unable to add break on exception for: %s (exception could not be imported).",
                               exception)
            return None
    ...
    def cancel_async_evaluation(self, thread_id, frame_id):
        with self._main_lock:
            try:
                all_threads = threadingEnumerate()
                for t in all_threads:
                    if getattr(t, 'is_pydev_daemon_thread', False) and hasattr(t,
                                                                               'cancel_event') and t.thread_id == thread_id and \
                            t.frame_id == frame_id:
                        t.cancel_event.set()
            except:
                pydev_log.exception()
    ...
    def do_wait_suspend(self, thread, frame, event, arg, exception_type=None):  # @UnusedVariable
        ...
        with self.suspended_frames_manager.track_frames(self) as frames_tracker:
            ...
            with CustomFramesContainer.custom_frames_lock:  # @UndefinedVariable
                ...
                for frame_custom_thread_id, custom_frame in CustomFramesContainer.custom_frames.items():
                    if custom_frame.thread_id == thread.ident:
                        frames_tracker.track(thread_id,
                        ...
                        self.writer.add_command(
                            frames_tracker.create_thread_suspend_command(frame_custom_thread_id, CMD_THREAD_SUSPEND, "",
                                                                         suspend_type))
            ...
            with self._threads_suspended_single_notification.notify_thread_suspended(thread_id, thread, stop_reason):
                keep_suspended = self._do_wait_suspend(thread, frame, event, arg, suspend_type, from_this_thread,
                                                       frames_tracker)
    ...
    def _do_wait_suspend(self, thread, frame, event, arg, suspend_type, from_this_thread, frames_tracker):
        ...
        if info.pydev_step_cmd in (CMD_STEP_INTO, CMD_STEP_INTO_MY_CODE):
            ...
            response_msg = ""
            try:
                stop, _old_line, response_msg = self.set_next_statement(frame, event, info.pydev_func_name,
    ...
    def run(self, file, globals=None, locals=None, is_module=False, set_trace=True):
        ...
        if self.thread_analyser is not None:
            ...
            send_concurrency_message("threading_event", 0, t.name, thread_id, "thread", "start", file, 1, None,
                                     parent=thread_id)

        if self.asyncio_analyser is not None:
            # we don't have main thread in asyncio graph, so we should add a fake event
            send_concurrency_message("asyncio_event", 0, "Task", "Task", "thread", "stop", file, 1, frame=None,
                                     parent=None)
...
def _log_initial_info():
    ...
    pydev_log.debug("Using gevent mode: %s / imported gevent module support: %s", SUPPORT_GEVENT,
